<?php

return [
	'success_send_feedback'         	=> 'Повідомлення успішно відправлено',
	'failed_send_feedback'          	=> 'Не вдалося відправити повідомлення',
	
	'method_not_found'                  => 'Метод не знайдено',
	'product_not_found'                 => 'Продукт не знайдено',
	
	'data_save_success'                 => 'Дані успішно збережено',
	'data_save_failed'                  => 'Неможливо зберегти дані',
	
	'you_need_to_log_in'                => 'Необхідно авторизуватись',
	
	'field_required'					=> 'Заповніть обов\'язкові дані',
	
	'failed_login'						=> 'Авторизація не вдалась',
	'success_login'						=> 'Авторизація пройшла успішно',
	'user_not_active'					=> 'Профіль цього користувача не підтверджено',
	
	'failed_register'					=> 'Реєстрація не вдалась',
	'success_register'					=> 'Реєстрація пройшла успішно',
	
	'user_not_found'					=> 'Користувача не знайдено',
	
	'failed_profile'					=> 'Помилка при оновленні профіля',
	'success_profile'					=> 'Профіль успішно оновлено',
	
	'code_invalid'						=> 'Вказаний код не знайдено',
	
	'failed_activation'					=> 'Активація не вдалась',
	'success_activation'				=> 'Активація пройшла успішно',
	
	'failed_change_password'			=> 'Зміна пароля не вдалась',
	'success_change_password'			=> 'Пароль успішно змінено',
	'passwords_not_match'				=> 'Паролі не співпадають',
	'password_incorrect'				=> 'Старий пароль невірний',
	
	'failed_add_order'					=> 'Не вдалось додати замовлення',
	'success_add_order'					=> 'Замовлення успішно додано',
	
	'failed_add_contract'				=> 'Не вдалось додати договір',
	'success_add_contract'				=> 'Договір успішно додано',
	
	'failed_add_address'				=> 'Не вдалось додати адресу',
	'success_add_address'				=> 'Адреса успішно додано',
		
	'failed_remove_address'				=> 'Не вдалось видалити адресу',
	'success_remove_address'			=> 'Адреса успішно видалена',
	
	'failed_send_sms'					=> 'Не вдалось відправити смс',
	
	'failed_add_address'				=> 'Не вдалось додати адресу',
	'success_add_address'				=> 'Адреса успішно додана',
	
	'contract_not_found'				=> 'Договір з вказаним номером не знайдено, зв\'яжіться будь ласка з менеджером',
	'data_do_not_match'					=> 'Дані вказані в договорі та дані контрагента не співпадають',
	'success_connect_contract'			=> 'Запит на приєднання договору відправлено на опрацювання менеджеру',
	
	'failed_create_request'				=> 'Не вдалось створити звернення',
	'select_theme'						=> 'Виберіть тему',
];
