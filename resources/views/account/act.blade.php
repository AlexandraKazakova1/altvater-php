@extends('account.layout')

@section('main')
	<main class="openContract__page">
		<section class="breadcrumbs__wrapper">
			<div class="breadcrumbs container">
				<ul class="breadcrumbs__list">
					<li class="breadcrumbs__item"><a href="/account">Кабінет</a></li>
					<li class="breadcrumbs__item"><a href="/account/bills">Мої рахунки</a></li>
					<li class="breadcrumbs__item"><a href="/account/bills">Акти</a></li>
					<li class="breadcrumbs__item active">
						<span>№{{$act->number}}</span>
					</li>
				</ul>
			</div>
		</section>
		
		<h1 class="page__title">{{$act->file_name}}</h1>
		
		<a class="back-link wb" href="/account/bills">
			<img src="/img/cabinet/arrow-red.svg">
			<span>Назад</span>
		</a>
		
		<section class="openAccounts">
			<div class="contract__info">
				<button type="botton" id="to_help">
					<svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M10.3242 9.68579C10.5612 8.66424 11.4917 7.95594 12.5395 7.99942C13.7121 7.93447 14.7174 8.82806 14.7904 10.0003C14.7904 11.5048 12.6395 12.0011 12.6395 13.0015" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
						<path d="M12.7659 15.7543C12.7658 15.8233 12.7098 15.8792 12.6408 15.8792C12.5718 15.8791 12.5158 15.8232 12.5158 15.7542C12.5158 15.6851 12.5717 15.6292 12.6407 15.6291C12.6739 15.629 12.7058 15.6422 12.7293 15.6657C12.7527 15.6892 12.7659 15.7211 12.7659 15.7543" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
						<path fill-rule="evenodd" clip-rule="evenodd" d="M5.26728 17.1605C2.58418 13.3308 3.27369 8.08546 6.85539 5.07926C10.4371 2.07305 15.7225 2.30347 19.029 5.60997C22.3355 8.91646 22.5659 14.2019 19.5597 17.7836C16.5535 21.3653 11.3082 22.0548 7.47847 19.3717L4.63555 20.0034L5.26728 17.1605Z" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
					</svg>
					Допомога
				</button>
			</div>
			
			<div class="account__wrapper">
				<embed src="/storage/{{$act->file}}" width="100%" height="1500px" />
			</div>
		</section>
	</main>
@stop
